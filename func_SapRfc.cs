...
{
    public class func_SapRfc
    {
        /* Variables globales */
        public Boolean estadoServicio = false;
        private System.Net.NetworkCredential Crendentials;

        private ff.dev.sap.rfc.ZK_WEBSERVICES_RELOJ_SN sap_service; //FF-DEV
        //private ff.dev.sap.rfc.ZK_WEBSERVICES_RELOJ_SN sap_service; //RFC QAS
        //private ff.dev.sap.rfc.ZK_WEBSERVICES_RELOJ_SN sap_service; //RFC PRD

        public func_SapRfc(String opAmbiente)
        {
            //se renueva la instancia de conexion a SAP
            sap_service = new rzkclock_ws.ff.dev.sap.rfc.ZK_WEBSERVICES_RELOJ_SN();
            ...
            if (estadoServicio)
            {
                try
                {
                    ...
                    sap_service.Url = wsSAP_url;
                    sap_service.Credentials = Crendentials;
                }
                catch (Exception E)
                {
        ...
        public DataTable obtenerItemRelojSAP(string ip)
        {
            //Boolean conexionSAP = estadoServicio ? estadoServicio : 
            if (estadoServicio)
            {
                ...
                ListaRelojesResponse = sap_service.ZkListaRelojes(ListaRelojes);
        ...
        public DataTable obtenerItemEmpleadoSAP(string cod_Empleado, string ip_GrupoReloj)
        {
            if (estadoServicio)
            {
                ...
                //Ejecuta consulta en SAP
                EmpleadosResponse = sap_service.ZkDistrEmpleados(Empleados);
        ...
        public DataTable obtenerItemHuellasEmpleadoSAP(string cod_Empleado, string ip_GrupoReloj)
        {
            if (estadoServicio)
            {
                ...
                //Ejecuta consulta en SAP
                HuellasResponse = sap_service.ZkEnviarHuellas(Huellas);
        ...
        public int servicioSAP_CargaMarcasDeReloj(DataTable ZKS_MARCAS)
        {
            if (estadoServicio)
            {
                ...
                MarcasEmpleadosResponse = sap_service.ZkMarcasEmpleados(MarcasEmpleados);
        ...
        public int servicioSAP_CargaHuellasDeReloj(DataTable ZKS_Huellas)
        {
            if (estadoServicio)
            {
                ...
                GHuellasResponse = sap_service.ZkGuardarHuellas(GHuellas);
                return 1;
            }
            else
            {
